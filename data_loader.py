...
class Collate:
    ...
    def collate_fn(self, batch):
        ...
        token_type_ids = torch.tensor(sequence_padding(batch_token_type_ids, length=self.maxlen), dtype=torch.long,
                                      device=self.device)
        batch_subject_labels = torch.tensor(sequence_padding(batch_subject_labels, length=self.maxlen),
                                            dtype=torch.float, device=self.device)
        batch_object_labels = torch.tensor(sequence_padding(batch_object_labels, length=self.maxlen), dtype=torch.float,
                                           device=self.device)
...
if __name__ == "__main__":
    ...
    # 当调用 from_pretrained 方法时，库会自动下载 vocab.txt 文件（以及其他必要的文件）到本地缓存
    train_dataset = MyDataset(file_path='data/ske/raw_data/train_data.json',
                              tokenizer=tokenizer,
                              max_len=max_len)
    # print(train_dataset[0])

    with open('data/ske/mid_data/predicates.json',encoding='utf-8') as fp:
        labels = json.load(fp)
    id2tag = {}
    tag2id = {}
